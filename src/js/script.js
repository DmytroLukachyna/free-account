document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!formContainer.parentNode.classList.contains('content-wrapper') && formContainer.parentNode.classList.contains('row')) {
    ...
    if (scrollToForm) {
        document.querySelectorAll(".scroll-to-form").forEach(el => el.addEventListener('click', () => {
    ...
    function phoneFieldOnlyDigits(e) {
        if ((e.key >= 0 && e.key <= 9) || (e.key === '+') || (e.key === '-') || (e.key === '/') || (e.key === '.') || (e.key === 'Backspace') || (e.key === 'Delete') || (e.key === 'ArrowLeft') || (e.key === 'ArrowRight') || (e.key === 'End') || (e.key === 'Home')) {
            return true;
        } else {
            e.preventDefault();
        }
